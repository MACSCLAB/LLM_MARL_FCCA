...
class R_Actor(nn.Module):
    ...
    def __init__(self, args, robot_obs_space, human_obs_space, action_space, device=torch.device("cpu")):
        ...
        self.robot_obs_shape = args.robot_obs_dim
        self.human_obs_shape = args.human_obs_dim
        self.obs_shape2network = self.robot_obs_shape - 2 # delete px,py of robot

        base = MLPBase
        self.base_robot = base(args, self.obs_shape2network)
        if self.use_lstm:
            self.base_human = base(args,) # Used to make the order of magnitude of both robot_feature and human_feature
        else:
            self.base_human = base(args, self.human_obs_shape * self.human_num)
    ...
    def forward(self, robot_obs, human_obs, rnn_states, masks, available_actions=None, deterministic=False):
        ...
        robot_obs, human_obs = transform(robot_obs, human_obs, self.human_num, self.att_agents)
        if self.use_lstm:
            human_features = self.lstm(human_obs)
            human_features = self.base_human(human_features)
        else:
            human_obs = human_obs.view(human_obs.shape[0],-1)
            human_features = self.base_human(human_obs)
        actor_features = self.base_robot(robot_obs)
        total_features = torch.cat((actor_features,human_features), dim=1)  #[thread 256]

        # if self._use_naive_recurrent_policy or self._use_recurrent_policy:
        #     actor_features, rnn_states = self.rnn(actor_features, rnn_states, masks)
        
        # 这是通过torch.distributions.Categorical的方法得到的
...
class R_Critic(nn.Module):
    ...
    def __init__(self, args, cent_obs_space, device=torch.device("cpu")):
        ...
        cent_obs_shape = (self.robot_num+self.att_agents) * (self.robot_obs_dim)
        base = MLPBase
    ...
    def forward(self, cent_obs, rnn_states, masks):
        ...
        :return rnn_states: (torch.Tensor) updated RNN hidden states.
        """
        #TODO 加多一层for循环来提取human特征，以适应不同障碍物的数量变化
        rnn_states = check(rnn_states).to(**self.tpdv)
        masks = check(masks).to(**self.tpdv)
        cent_obs = check(cent_obs).to(**self.tpdv)
        batch = cent_obs.shape[0]
        cent_obs = cent_obs.view(batch,-1)
        cent_obs = cent_obs[:,:(self.att_agents+self.robot_num)*(self.robot_obs_dim)]
